...
export const borderStyle = (thickness: number) => css`
  border: ${thickness}px solid rgba(${(props) => props.theme.yellow_green}, 0.8);
`;

export const themeColor = (color: string, opacity: number) => css`
  color: rgba(${(props) => props.theme[color]}, ${opacity});
`;

export const themeBackgroundColor = (color: string, opacity: number) => css`
...
export const imageBase = (w: number, h: number) => css`
  width: ${w}px;
  height: ${h}px;
  border-radius: 50%;
  ${borderStyle(3)};
